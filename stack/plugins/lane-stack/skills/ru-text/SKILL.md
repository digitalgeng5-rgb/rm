...
name: ru-text
description: >
  [RU] Вызывай, когда работа идёт над качеством русского текста: вычитка,
  типографика, очистка от нейрослопа, редактура, UX-тексты, деловая переписка —
  или по прямому упоминанию ru-text. Typography silently on any Russian output;
  deeper edit on request. SKIP: SEO keys (seo-specialist); detector-evasion.
metadata:
  openclaw:
    always: true
    emoji: "📝"
...
Call this skill whenever the work is Russian text quality: вычитка, типографика,
нейрослоп, редактура, UX-тексты, деловая переписка — or the human said `ru-text`.
Any agent. Not only copy-lead. `ru-check` = findings list. `ru-score` = 0–10.
